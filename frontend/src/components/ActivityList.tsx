...
import api from "../lib/api"; // <--- Importamos nuestra instancia centralizada
...
export const ActivityList = ({ taskId }: Props) => {
	const { data: logs, isLoading } = useQuery({
		...
		queryFn: async () => (await api.get(`/tasks/${taskId}/activity`)).data, // <--- Ruta relativa limpia
		refetchInterval: 5000, // Refrescar auto para ver movimientos de otros usuarios en tiempo real
	});
